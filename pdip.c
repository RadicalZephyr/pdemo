...
static void pdip_flush(int fd)
{
    ...
    do
    {
        rc = pdip_read(fd, pdip_buf, pdip_bufsz);
        if (rc > 0)
        {
 ...
 )
{
    ...
    *p = '\0';
    ...
    if (0 != rc)
    {
        PDIP_ERR("Bad regular expression\n");
        return;
    }
                ...
                if (fd_program >= 0)
                {
                    if (FD_ISSET(fd_program, &fdset))
                    {
                        ...
                        if (synchro)
                        {
                            ...
                            PDIP_DUMP(2, pdip_outstanding_buf, pdip_loutstanding);
         ...
         )
{
    ...
    while ((opt = getopt_long(ac, av, "s:b:d:Vetoph", pdip_longopts, NULL)) != EOF)
    {
        switch(opt)
        {
